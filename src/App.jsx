...
function HomePage() {
  ...
  return <><ScrollProgress /><a className="skip-link" href="#main">Skip to content</a><Header /><main id="main">
    ...
    <section className="skills-section"><div className="shell"><SectionIntro index="05" label="Systems" light title="Technical range, organised by responsibility."/><div className="skill-map">{skillGroups.map(([group, skills], index) => <article key={group}><span>0{index + 1}</span><h3>{group}</h3><ul>{skills.map(skill => <li key={skill}>{skill}</li>)}</ul></article>)}</div></div></section>
